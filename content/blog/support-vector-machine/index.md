...
The samples for which the constraint holds with equality are called **support vectors**.
They are the only points that touch the margin and (as we will see in the dual) the only points whose data actually shapes the solution.
...
We relax the constraint $y\_i z\_i \geq 1$ by allowing each sample to violate the margin by a non-negative slack $\xi\_i \geq 0$, then pay a linear penalty $C \xi\_i$ for the violation.
...
It is zero whenever the score lands on the correct side of the margin ($y z \geq 1$), grows linearly into the margin and across the boundary, and has a non-differentiable kink at $y z = 1$.
...
**Pegasos** (Shalev-Shwartz, Singer, and Srebro, 2007) is the canonical example: at iteration $t$ pick a random sample $i\_t$, set the step size $\eta\_t = 1 / (\lambda t)$ with $\lambda = 1/(NC)$, and update
...
**Sequential Minimal Optimisation** (SMO; Platt, 1998) is a coordinate-ascent method for {{ eqref(id="svm-dual") }} that exploits a clever observation.
The equality constraint $\sum\_i \alpha\_i y\_i = 0$ means we cannot update a single $\alpha\_i$ in isolation, since changing it breaks the constraint.
...
See Nesterov, _Lectures on Convex Optimization_, §3.2. $\square$
{% end %}
...
Original analysis in Shalev-Shwartz, Singer, and Srebro (2007), tightened in Rakhlin, Shamir, and Sridharan (2012) to remove the log factor under specific averaging schemes. $\square$
{% end %}
...
Hsieh, Chang, Lin, Keerthi, and Sundararajan (2008) gave the first sharp linear-rate analysis for SVM dual coordinate descent, and the result underpins LIBLINEAR's default solver for $L\_2$-loss linear SVM. $\square$
{% end %}
...
**The choice of $C$** controls the bias-variance trade-off and has no good default: it must be tuned by cross-validation, ideally on a logarithmic grid $C \in \\{10^{-3}, 10^{-2}, \ldots, 10^{3}\\}$.
...
The standard remedies are sub-sampling, the Nyström approximation, or random Fourier features (Rahimi and Recht, 2007), which approximate the kernel by an explicit low-dimensional embedding and reduce the problem back to a linear SVM.
{% end %}
...
See Schölkopf and Smola, _Learning with Kernels_, Chapter 2. $\square$
{% end %}
...
The **radial basis function** (RBF) kernel $k(\mathbf{x}, \mathbf{x}') = \exp(-\gamma \lVert \mathbf{x} - \mathbf{x}' \rVert\_2^2)$ is the workhorse default for low-dimensional dense data: its implicit feature space is infinite-dimensional, and the bandwidth $\gamma$ controls how quickly similarity decays with Euclidean distance.
...
The identity $\mathbf{w}^{\ast} = \sum\_i \alpha\_i y\_i \phi(\mathbf{x}\_i)$ is a special case of a more general result, the **representer theorem** (Kimeldorf and Wahba, 1971; Schölkopf, Herbrich, and Smola, 2001).
...
This is what justifies dual formulations more broadly, not just for SVMs.
{% end %}
...
**Random Fourier features** (Rahimi and Recht, 2007) and the **Nyström approximation** (Williams and Seeger, 2001) sidestep this by approximating $k(\mathbf{x}, \mathbf{x}') \approx \phi\_R(\mathbf{x})^{\top} \phi\_R(\mathbf{x}')$ for an explicit $R$-dimensional feature map $\phi\_R$, after which a linear SVM solver suffices.
{% end %}
...
The full derivation is in Crammer and Singer (2001) and is omitted here.
